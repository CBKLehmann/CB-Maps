...
class Map2_0(Map2_0Template):
    ...
    def form_show(self, **event_args):
      ...
      self.mapbox.on('mousemove', 'gemeinden', self.mousemove)
      self.mapbox.on('mouseleave', 'gemeinden', self.mouseleave)
...
#             geojson = anvil.server.call('poi_data', 'doctors')
...
#           el.remove()
    ...
    def check_box_doc_change(self, **event_args):
      ...
      Variables.last_bbox_den = self.create_icons(self.check_box_doc.checked, Variables.last_bbox_doc, 'doctors', 'https://wiki.openstreetmap.org/w/images/7/71/Doctors-14.svg')
    ...
    def check_box_den_change(self, **event_args):
      ...
      if self.check_box_den.checked == True:
        ...
        if not bbox == Variables.last_bbox_den:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_den[0] or bbox[1] < Variables.last_bbox_den[1] or bbox[2] > Variables.last_bbox_den[2] or bbox[3] > Variables.last_bbox_den[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'dentist')
            ...
            for ele in geojson:
              ...
              if bbox[0] < el_coords[1] < bbox[2] and bbox[1] < el_coords[0] < bbox[3]:
                ...
                #Create Icon
                el.className = 'icon_doc'
                el.style.backgroundImage = f'url(https://wiki.openstreetmap.org/w/images/8/86/Dentist-14.svg)'
            ...
            #Refresh global Variables
            Variables.icons.update({'dentist': icons})
            Variables.last_bbox_den = bbox
            Variables.last_cat = 'dentist'
          
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['dentist']:
    ...
    def check_box_cli_change(self, **event_args):
      ...
      if self.check_box_cli.checked == True:
        ...
        if not bbox == Variables.last_bbox_cli:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_cli[0] or bbox[1] < Variables.last_bbox_cli[1] or bbox[2] > Variables.last_bbox_cli[2] or bbox[3] > Variables.last_bbox_cli[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'clinic')
            ...
            #Refresh global Variables
            Variables.icons.update({'clinic': icons})
            Variables.last_bbox_cli = bbox
            Variables.last_cat = 'clinic'
        
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['clinic']:
    ...
    def check_box_hos_change(self, **event_args):
      ...
      if self.check_box_hos.checked == True:
        ...
        if not bbox == Variables.last_bbox_hos:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_hos[0] or bbox[1] < Variables.last_bbox_hos[1] or bbox[2] > Variables.last_bbox_hos[2] or bbox[3] > Variables.last_bbox_hos[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'hospital')
            ...
            for ele in geojson:
              ...
              if bbox[0] < el_coords[1] < bbox[2] and bbox[1] < el_coords[0] < bbox[3]:
                ...
                #Create Icon
                el.className = 'icon_doc'
                el.style.backgroundImage = f'url(https://wiki.openstreetmap.org/w/images/3/33/Hospital-14.svg)'
            ...
            #Refresh global Variables
            Variables.icons.update({'hospital': icons})
            Variables.last_bbox_hos = bbox
            Variables.last_cat = 'hospital'
        
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['hospital']:
    ...
    def check_box_nur_change(self, **event_args):
      ...
      if self.check_box_nur.checked == True:
        ...
        if not bbox == Variables.last_bbox_nur:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_nur[0] or bbox[1] < Variables.last_bbox_nur[1] or bbox[2] > Variables.last_bbox_nur[2] or bbox[3] > Variables.last_bbox_nur[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'nursing_home')
            ...
            #Refresh global Variables
            Variables.icons.update({'nursing-home': icons})
            Variables.last_bbox_nur = bbox
            Variables.last_cat = 'nursing-home'
        
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['nursing-home']:
    ...
    def check_box_pha_change(self, **event_args):
      ...
      if self.check_box_pha.checked == True:
        ...
        if not bbox == Variables.last_bbox_pha:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_pha[0] or bbox[1] < Variables.last_bbox_pha[1] or bbox[2] > Variables.last_bbox_pha[2] or bbox[3] > Variables.last_bbox_pha[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'pharmacy')
            ...
            for ele in geojson:
              ...
              if bbox[0] < el_coords[1] < bbox[2] and bbox[1] < el_coords[0] < bbox[3]:
                ...
                #Create Icon
                el.className = 'icon_doc'
                el.style.backgroundImage = f'url(https://wiki.openstreetmap.org/w/images/1/1e/Pharmacy-14.svg)'
            ...
            #Refresh global Variables
            Variables.icons.update({'pharmacy': icons})
            Variables.last_bbox_pha = bbox
            Variables.last_cat = 'pharmacy'
        
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['pharmacy']:
    ...
    def check_box_soc_change(self, **event_args):
      ...
      if self.check_box_soc.checked == True:
        ...
        if not bbox == Variables.last_bbox_soc:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_soc[0] or bbox[1] < Variables.last_bbox_soc[1] or bbox[2] > Variables.last_bbox_soc[2] or bbox[3] > Variables.last_bbox_soc[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'social_facility')
            ...
            for ele in geojson:
              ...
              if bbox[0] < el_coords[1] < bbox[2] and bbox[1] < el_coords[0] < bbox[3]:
                ...
                #Create Icon
                el.className = 'icon_doc'
                el.style.backgroundImage = f'url(https://wiki.openstreetmap.org/w/images/0/0e/Social_facility-14.svg)'
            ...
            #Refresh global Variables
            Variables.icons.update({'social_facility': icons})
            Variables.last_bbox_soc = bbox
            Variables.last_cat = 'social_facility'
        
          else:
          
            #Loop through every Element in global Icon-Elements
            for el in Variables.icons['social_facility']:
    ...
    def check_box_vet_change(self, **event_args):
      ...
      if self.check_box_vet.checked == True:
        ...
        if not bbox == Variables.last_bbox_vet:
          ...
          #Check if new Bounding Box is overlapping old Bounding Box
          if bbox[0] < Variables.last_bbox_vet[0] or bbox[1] < Variables.last_bbox_vet[1] or bbox[2] > Variables.last_bbox_vet[2] or bbox[3] > Variables.last_bbox_vet[3]:
          
            #Get geojson of POIs inside Bounding Box
            geojson = anvil.server.call('poi_data', 'veterinary')
            ...
            #Refresh global Variables
            Variables.icons.update({'veterinary': icons})
            Variables.last_bbox_vet = bbox
            Variables.last_cat = 'veterinary'
